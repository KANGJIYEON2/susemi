...
회사가 신고한 결과와 단계별로 cross-check 까지 해줘서, 환급액이 새는 곳을 잡아냅니다.
...
| 보안 | 기본 | **HMAC 토큰 인증 + slowapi rate limit + path traversal 차단 + eval() 0줄** |
...
### 6. Greedy 추천 — What-if 4 lever (`POST /api/v1/recommend`)

| Lever | 한도 | 설명 |
|---|---|---|
| 연금저축 | 400만 | 세액공제 12~15% |
| IRP | 합산 700만 | 연금저축+IRP 합산 한도 |
| 정치자금 | 10만 | 10만 이하 전액 공제 |
| 월세 세액공제 | - | 세대주 + 무주택 + 임대차 조건 |
